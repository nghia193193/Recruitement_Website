...
import { verifyAccessToken, isPDF } from '../utils';
...
export const candidateController = {
    ...
    checkApply: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const authHeader = req.get('Authorization') as string;
            const accessToken = authHeader.split(' ')[1];
            const decodedToken: any = await verifyAccessToken(accessToken);
    ...
    getAppliedJobs: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const authHeader = req.get('Authorization') as string;
            const accessToken = authHeader.split(' ')[1];
            const decodedToken: any = await verifyAccessToken(accessToken);
    ...
    addFavoriteJob: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const authHeader = req.get('Authorization') as string;
            const accessToken = authHeader.split(' ')[1];
            const decodedToken: any = await verifyAccessToken(accessToken);
            ...
            await candidateService.addFavoriteJob(candidateId, jobId);
            res.status(200).json({ success: true, message: "Add job to favorite successfully!" });
        } catch (error) {
            next(error);
        }
    },
    getFavoriteJob: async (req: Request, res: Response, next: NextFunction): Promise<void> => {
        try {
            const authHeader = req.get('Authorization') as string;
            const accessToken = authHeader.split(' ')[1];
            const decodedToken: any = await verifyAccessToken(accessToken);
            const candidateId = decodedToken.userId;
            const favoriteJobs = await candidateService.getFavoriteJob(candidateId);
