...
export const recruiterService = {
    getAllJobs: async (recruiterId: string, name: any, type: any, position: any,
    ...
    createJob: async (recruiterId: string, name: string, position: string, jobType: string, quantity: string,
    ...
    getSingleJob: async (recruiterId: string, jobId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    updateJob: async (recruiterId: string, jobId: string, name: string, position: string, jobType: string,
    ...
    deleteJob: async (recruiterId: string, jobId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllEvents: async (recruiterId: string, query: any, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleEvent: async (recruiterId: string, eventId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    createEvent: async (recruiterId: string, image: UploadedFile, title: string, name: string,
    ...
    updateEvent: async (recruiterId: string, eventId: string, image: any, title: string, name: string,
        description: string, time: string, location: string, deadline: any, startAt: any) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
    ...
    deleteEvent: async (recruiterId: string, eventId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllInterviewers: async (recruiterId: string, name: any, skill: any, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleInterviewer: async (recruiterId: string, interviewerId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllApplicants: async (recruiterId: string, name: any, skill: any, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleApplicant: async (recruiterId: string, applicantId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getApplicantsJob: async (recruiterId: string, jobId: string, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleApplicantJob: async (recruiterId: string, jobId: string, candidateId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    createMeeting: async (recruiterId: string, candidateId: string, interviewersId: any, time: any, jobApplyId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    updateCandidateState: async (recruiterId: string, candidateId: string, jobId: string, state: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getJobSuggestedCandidates: async (recruiterId: string, jobId: string, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getInterviewsOfCandidate: async (recruiterId: string, candidateId: string, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
        ...
        const returnInterviews = interviews.map(interview => {
            ...
            return {
                ...
                position: interview.jobs[0].potition,
    ...
    getInterviewsOfInterviewer: async (recruiterId: string, interviewerId: string, page: number, limit: number) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    recruiterStatistics: async (recruiterId: string) => {
        const recruiter = await User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error: Error & { statusCode?: any, result?: any } = new Error('Không tìm thấy user');
            error.statusCode = 409;
