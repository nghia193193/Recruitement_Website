...
export const getResumes = async (candidateId: string) => {
...
export const uploadResume = async (candidateId: string, resume: UploadedFile) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const deleteResume = async (candidateId: string, resumeId: string) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const checkApply = async (candidateId: string, jobId: string) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const applyJob = async (candidateId: string, jobId: string, resumeId: string) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const getAppliedJobs = async (candidateId: string, page: number, limit: number) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const saveInformation = async (candidateId: string, education: any, experience: any, certificate: any, project: any, skills: any) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const getInformation = async (candidateId: string) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
...
export const getAllInterviews = async (candidateId: string, page: number, limit: number) => {
    const candidate = await User.findById(candidateId).populate('roleId');
    if (candidate?.get('roleId.roleName') !== 'CANDIDATE') {
        const error: Error & { statusCode?: number, result?: any } = new Error('UnAuthorized');
        error.statusCode = 401;
