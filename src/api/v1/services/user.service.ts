...
export const updateProfile = async (userId: string, fullName: string, address: string, dateOfBirth: any, about: string) => {
...
export const changeAvatar = async (userId: string, avatar: any) => {
    const user = await User.findById(userId) as any;
    if (!user) {
        const error: Error & { statusCode?: number, result?: any } = new Error('Không tìm thấy user');
        error.statusCode = 409;
