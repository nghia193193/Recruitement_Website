...
exports.recruiterService = {
    getAllJobs: async (recruiterId, name, type, position, location, active, page, limit) => {
    ...
    createJob: async (recruiterId, name, position, jobType, quantity, benefit, salaryRange, requirement, location, description, deadline, skillRequired) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleJob: async (recruiterId, jobId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    updateJob: async (recruiterId, jobId, name, position, jobType, quantity, benefit, salaryRange, requirement, location, description, deadline, skillRequired) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllEvents: async (recruiterId, query, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleEvent: async (recruiterId, eventId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    createEvent: async (recruiterId, image, title, name, description, time, location, deadline, startAt) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    updateEvent: async (recruiterId, eventId, image, title, name, description, time, location, deadline, startAt) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    deleteEvent: async (recruiterId, eventId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllInterviewers: async (recruiterId, name, skill, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleInterviewer: async (recruiterId, interviewerId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getAllApplicants: async (recruiterId, name, skill, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleApplicant: async (recruiterId, applicantId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
        ...
        ;
        const applicant = await user_1.User.findById(applicantId).populate('roleId');
        if (!applicant) {
    ...
    getApplicantsJob: async (recruiterId, jobId, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getSingleApplicantJob: async (recruiterId, jobId, candidateId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    createMeeting: async (recruiterId, candidateId, interviewersId, time, jobApplyId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    updateCandidateState: async (recruiterId, candidateId, jobId, state) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getJobSuggestedCandidates: async (recruiterId, jobId, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getInterviewsOfCandidate: async (recruiterId, candidateId, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    getInterviewsOfInterviewer: async (recruiterId, interviewerId, page, limit) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
    ...
    recruiterStatistics: async (recruiterId) => {
        const recruiter = await user_1.User.findById(recruiterId).populate('roleId');
        if (!recruiter) {
            const error = new Error('Không tìm thấy user');
            error.statusCode = 409;
