...
exports.adminService = {
    getAllAccounts: async (adminId, searchText, searchBy, active, page, limit) => {
    ...
    getAllRecruiterAccounts: async (adminId, searchText, searchBy, active, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    getAllInterviewerAccounts: async (adminId, searchText, searchBy, active, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    getAllCandidateAccounts: async (adminId, searchText, searchBy, active, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    getAllBlackListAccounts: async (adminId, searchText, searchBy, active, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    addBlackList: async (adminId, userId) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    removeBlackList: async (adminId, candidateId) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    createAccount: async (adminId, fullName, email, password, phone, position) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    getAllJobs: async (adminId, recruiterName, jobName, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
    ...
    getAllEvents: async (adminId, recruiterName, eventName, page, limit) => {
        const admin = await user_1.User.findById(adminId).populate('roleId');
        if (admin?.get('roleId.roleName') !== 'ADMIN') {
            const error = new Error('UnAuthorized');
            error.statusCode = 401;
